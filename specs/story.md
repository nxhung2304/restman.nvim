...
# 🚀 Tệp Đặc tả (Spec) — `restman.nvim` (v1.0 MVP)
...
* **Lệnh tương ứng (cho user không dùng keymap):** dùng cú pháp **subcommand** với một command gốc `:Restman`:
  * `:Restman send`
  * `:Restman repeat`
  * `:Restman env` — chọn environment active
  * `:Restman history`
  * `:Restman cancel`
  * `:Restman rails` — mở Rails routes picker
  * `:Restman rails refresh` — invalidate cache routes
  * `:Restman health` — alias của `:checkhealth restman`
...
1. **Visual selection** (ưu tiên cao nhất) — nếu user có active visual selection khi bấm `<leader>rs`, block đó luôn thắng, kể cả khi có `@restman.body` directive gần đó.
2. **Heredoc comment ngay trên dòng request** — parser đọc comment block liền kề:
   ...
   ```
   Các directive hỗ trợ: `@restman.body`, `@restman.header`, `@restman.query`, `@restman.form`.
   **Lý do chọn namespace `@restman.*`:** tránh xung đột với JSDoc (`@body`), YARD (`@param`), phpDoc, hoặc các plugin comment-directive khác. Parser chỉ nhận diện prefix `@restman.` — các directive không namespace sẽ bị bỏ qua.
...
> **Headers merge khác với body:** headers từ env (§3.1) luôn merge vào request, chỉ bị override khi key trùng trong `@restman.header`. Tức là headers là **merge**, còn body là **winner-take-all**.
...
* **Buffer layer:** Mỗi response được ghi vào một **scratch buffer** riêng, đặt tên dạng `restman://response/<n>` (n tăng dần). Buffer là `nofile`, `bufhidden=hide`, không save được — đóng view không xóa data.
...
* `:Restman send` → mở bằng `default_view` trong config.
* `:Restman history` → **mở bằng split** (user đang inspect sâu, không phải quick peek). Override được qua `history.view = "float"`.
...
* **Cancel:** `:Restman cancel` hoặc `<leader>rc` giết process đang chạy. Bấm `<leader>rs` lần 2 khi chưa có response → prompt "Cancel previous request? [y/N]".
...
* **History list:**
  * Lưu tại `vim.fn.stdpath("data") .. "/restman/history.json"`.
...
* **Cache** tại `.cache/restman/rails_routes.txt` (trong project root, thêm vào `.gitignore`).
* Lệnh `:Restman rails refresh` để invalidate cache (khi user vừa thêm route mới).
...
* **Stale detection (nhẹ):** mỗi lần `:Restman rails` đọc cache, so sánh `mtime` của `config/routes.rb` với `mtime` của cache file. Nếu routes.rb mới hơn → log warning `"routes.rb has changed since last cache, run :Restman rails refresh"` nhưng **vẫn dùng cache cũ** (không block user).
* Parse output → list `{ verb, path, controller#action }` → đưa vào picker.
* Chọn → nếu có `:param` thì prompt → gửi.
* **Giới hạn với Grape:** nếu app dùng mounted API kiểu `mount Api::Base => "/"`, `bin/rails routes` có thể **không bung đầy đủ** endpoint bên trong Grape. Khi đó `:Restman rails` chỉ thấy mount point hoặc route tổng quát, không đủ để picker toàn bộ request con.

#### 7.1.1 `:Restman rails grape` (future)
* Mục tiêu: hỗ trợ app Rails có mounted Grape API, nơi `bin/rails routes` không phản ánh đầy đủ endpoint thực.
* Command riêng: `:Restman rails grape`.
* Behavior tối thiểu:
  * Detect mounted Grape API từ `config/routes.rb`.
  * Nếu user gọi `:Restman rails` trên project có Grape mount → notify warning rõ ràng rằng danh sách route có thể incomplete.
  * `:Restman rails grape` sẽ là entrypoint riêng để introspect hoặc parse endpoint Grape theo cơ chế khác với `bin/rails routes`.
* **Không nằm trong MVP v1.0.** Nếu chưa implement, fallback chính thức là user viết request line thủ công rồi dùng `:Restman send`.
...
* Telescope extension: `:Telescope restman history`, `:Telescope restman env`, `:Telescope restman rails_routes`.
...
│   └── directives.lua -- @restman.body, @restman.header comment directives
...
- [ ] Parse directive `@restman.body`, `@restman.header`, `@restman.query`, `@restman.form` trong comment liền kề.
...
- [ ] Gửi POST với body từ `@restman.body` directive trong comment liền kề.
...
### UI
- [ ] Response ghi vào **scratch buffer** `restman://response/<n>`, không phải render trực tiếp vào window.
...
- [ ] `:Restman history` mở entry bằng split (không phải float).
...
### Rails
- [ ] `:Restman rails` load và cache `rails routes`.
- [ ] `:Restman rails refresh` invalidate cache.
...
- [ ] `:checkhealth restman` báo trạng thái: Neovim version ≥ 0.10, curl version, env loaded, telescope available.
